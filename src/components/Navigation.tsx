  ...
  return (
    <>
          ...
          <div className="hidden md:flex items-center gap-10">
            {navLinks.map((link) => (
              <button
                key={link.href}
                onClick={() => handleClick(link.href)}
                className="text-[13px] font-sans font-bold tracking-widest uppercase transition-colors duration-300 relative py-1 group text-white hover:text-white/80"
              >
                {link.label}
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="h-[50px] px-8 bg-white hover:bg-white/90 text-brand-teal-dark text-xs uppercase tracking-widest font-semibold inline-flex items-center justify-center transition-all duration-300 rounded-[8px]"
            >
              Book Now
            </a>
          </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            className="fixed inset-0 z-40 bg-brand-teal-dark flex flex-col justify-between px-8 py-24 md:hidden"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
          >
            <div className="flex flex-col gap-8 mt-10">
              {navLinks.map((link, i) => (
                <motion.button
                  key={link.href}
                  onClick={() => handleClick(link.href)}
                  className="text-left text-3xl font-display font-bold text-white hover:text-white/80 transition-colors tracking-wide"
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: i * 0.08 }}
                >
            ...
            >
              <div className="h-px bg-white/20" />
                ...
                rel="noopener noreferrer"
                onClick={() => setMobileOpen(false)}
                className="w-full h-[52px] bg-white hover:bg-white/90 text-brand-teal-dark text-xs uppercase tracking-widest font-semibold inline-flex items-center justify-center transition-all duration-300 rounded-[8px]"
              >
                Book Now
              </a>
            </motion.div>
